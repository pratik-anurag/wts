...
func TestLoadReleaseFallsBackToNewestPreview(t *testing.T) {
	...
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		case "/repos/pratik-anurag/wts/releases?per_page=1":
			...
			response.Body = io.NopCloser(strings.NewReader(`[{"tag_name":"v0.1.4","prerelease":true,"assets":[]}]`))
		default:
	...
	if result.TagName != "v0.1.4" || !result.Prerelease {
		t.Fatalf("loadRelease() = %#v, want the v0.1.4 preview", result)
	}
	wantRequests := []string{
