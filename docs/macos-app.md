...
For WTS version `0.1.4`, output is:

```text
target/release/bundle/macos/WTS.app
target/release/bundle/dmg/WTS_0.1.4_aarch64.dmg
target/release/bundle/dmg/WTS_0.1.4_aarch64.dmg.sha256
```
...
signed Apple Silicon QA app archive on pushes to `wts-ui` and manual runs. A tag
matching the desktop version, such as `v0.1.4`, takes the release path and
...
gh release download v0.1.4 --pattern '*.dmg' --pattern '*.sha256' --dir "$PWD/wts-download"
cd "$PWD/wts-download"
...
`WTS-macOS-arm64-<commit>` artifact. Verify its checksum before you extract the
app archive. QA artifacts use an ad-hoc signature. macOS can require approval
in **Privacy & Security**. Tagged previews also use an ad-hoc signature when
Apple release credentials are absent.
